...
use App\Models\Time;
use App\Models\OtherMlConstruction;
class OtherYearlyMlConstruction extends Controller
{
    public function index()
    {
        ...
        $data['data'] = OtherMlConstruction::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->where('year',$data['get_time_data']->year)->orderBy('id','desc')->get();
        return view('ml_domestic_construction_yearly.other.index',$data);
    }
    ...
    public function insert(Request $request)
    {
        ...
        {
        $production = new OtherMlConstruction;
    ...
    {
        $data = [];
        $data['data'] = OtherMlConstruction::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('ml_domestic_construction_yearly.other.edit',$data);
        }else{
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        OtherMlConstruction::where('id',$request->id)->update($production);
