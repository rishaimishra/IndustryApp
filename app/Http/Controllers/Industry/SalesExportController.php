...
class SalesExportController extends Controller
{
    public function index()
    {
        ...
        $data['data'] = SalesExport::where('status','!=','D')->where('type','H')->where('user_id',auth()->user()->id)->orderBy('id','desc')->where('profile_id',auth()->user()->current_profile)->where('year',$year)->get();
    ...
    public function save_next(Request $request)
    {
        ...
        if (@$request->type_submission=="S") {
            ...
            return redirect()->route('manage.raw.material');
        }  
    }
    ...
    {
        $data = [];
        $data['data'] = SalesExport::where('id',$id)->where('type','H')->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        if($data['data']!="")
        {
