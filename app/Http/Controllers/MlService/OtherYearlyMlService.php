...
use App\Models\Time;
use App\Models\OtherMlService;
class OtherYearlyMlService extends Controller
{
    public function index()
    {
        ...
        $data['data'] = OtherMlService::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->where('year',$data['get_time_data']->year)->orderBy('id','desc')->get();
        return view('ml_service_yearly.other.index',$data);
    }
    ...
    {
        $data = [];
        $data['data'] = OtherMlService::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('ml_service_yearly.other.edit',$data);
        }else{
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        OtherMlService::where('id',$request->id)->update($production);
