...
use App\Models\Time;
use App\Models\RevenueCsiConstructions;
class RevenueYearlyCsiConstruction extends Controller
{
        public function index()
    {
        ...
        $data['data'] = RevenueCsiConstructions::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->orderBy('id','desc')->where('year',$data['get_time_data']->year)->get();
        // return $data['data'];
        return view('csi_construction_yearly.revenue.index',$data);
    }
    ...
    public function insert(Request $request)
    {
        ...
        {
        $production = new RevenueCsiConstructions;
        ...
        if (@$request->type_submission=="S") {
            ...
            return redirect()->route('manage.other-csi-construction-yearly');
        }
    }
    ...
    {
        $data = [];
        $data['data'] = RevenueCsiConstructions::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('csi_construction_yearly.revenue.edit',$data);
        }else{
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        RevenueCsiConstructions::where('id',$request->id)->update($production);
