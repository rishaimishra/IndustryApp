...
use App\Models\Time;
use App\Models\MlPamOther;
class OtherMl extends Controller
{
    public function index()
    {
        ...
        $data['data'] = MlPamOther::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->where('year',$data['get_time_data']->year)->orderBy('id','desc')->get();
        return view('ml_domestic_pam_yearly.other.index',$data);
    }
    ...
    public function add()
    {
        ...
        $data['get_time_data'] = Time::whereDate('from_date', '<=', date('Y-m-d'))
            ->whereDate('end_date', '>=',date('Y-m-d'))->where('type','Y')->first();
        return view('ml_domestic_pam_yearly.other.add',$data);
    }
    ...
    public function insert(Request $request)
    {
        ...
        if (@$request->type_submission=="S") {
            ...
            return redirect()->route('manage.half.yearly.submission.ml.pam.domestic.yearly');
        }
    }
    ...
    {
        $data = [];
        $data['data'] = MlPamOther::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        if($data['data']!="")
        {
            return view('ml_domestic_pam_yearly.other.delete',$data);
        }else{
    ...
    {
        MlPamOther::where('id',$request->id)->update([
            'status'=>'D',
    ...
    public function edit($id)
    {
        ...
        if($data['data']!="")
        {
            return view('ml_domestic_pam_yearly.other.edit',$data);
        }else{
            return redirect()->back()->with('error','Something Went Wrong');
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        MlPamOther::where('id',$request->id)->update($production);
