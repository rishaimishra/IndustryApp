...
use App\Models\Time;
class SalesDomesticMlPam extends Controller
{
    public function index()
    {
        ...
        $data['data'] = MlpumSalesDomestic::where('status','!=','D')->where('type','H')->where('user_id',auth()->user()->id)->orderBy('id','desc')->where('profile_id',auth()->user()->current_profile)->where('year',$year)->get();
        return view('ml_domestic_pam_half.sales_domestic.index',$data);
    }
    ...
    public function save_next(Request $request)
    {
        ...
        if (@$request->type_submission=="S") {
            ...
            return redirect()->route('manage.sales.export.ml.pam.domestic');
        }  
    }
    ...
    {
        $production = new MlpumSalesDomestic;
    ...
    {
        $data = [];
        $data['data'] = MlpumSalesDomestic::where('id',$id)->where('type','H')->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        if($data['data']!="")
        {
    ...
    {
        MlpumSalesDomestic::where('id',$request->id)->update([
            'status'=>'D',
    ...
    public function edit($id)
    {
        ...
        if($data['data']!="")
        {
            return view('ml_domestic_pam_half.sales_domestic.edit',$data);
        }else{
            return redirect()->back()->with('error','Something Went Wrong');
        }
    }
