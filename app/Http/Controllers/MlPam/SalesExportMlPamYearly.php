...
use App\Models\Time;
class SalesExportMlPamYearly extends Controller
{
    public function index()
    {
        ...
        $data['data'] = MlpumSalesExport::where('status','!=','D')->where('type','Y')->where('user_id',auth()->user()->id)->orderBy('id','desc')->where('profile_id',auth()->user()->current_profile)->where('year',$year)->get();
        $data['country']= Country::get();
        return view('ml_domestic_pam_yearly.sales_export.index',$data);
    }
    ...
    public function save_next(Request $request)
    {
        ...
        if (@$request->type_submission=="S") {
            ...
            return redirect()->route('manage.raw.material.ml.pam.domestic.yearly');
        }  
    }
    ...
    {
        $data = [];
        $data['data'] = MlpumSalesExport::where('id',$id)->where('type','Y')->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        $data['products'] = MlpumProduction::where('status','!=','D')->where('user_id',auth()->user()->id)->get();
        $data['country']= Country::get();
        if($data['data']!="")
        {
            return view('ml_domestic_pam_yearly.sales_export.edit',$data);
        }else{
