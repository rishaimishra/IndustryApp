...
use App\Models\Time;
use App\Models\MlPamFinance;
class FinanceMl extends Controller
{
    public function index()
    {
        ...
        $data['data'] = MlPamFinance::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->where('year',$data['get_time_data']->year)->orderBy('id','desc')->get();
        return view('ml_domestic_pam_yearly.finance.index',$data);
    }
    ...
    public function insert(Request $request)
    {
        ...
        if (@$request->type_submission=="S") {
            ...
            return redirect()->route('manage.revenue.ml-domestic-pam');
        }
    }
    ...
    {
        $data = [];
        $data['data'] = MlPamFinance::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('ml_domestic_pam_yearly.finance.edit',$data);
        }else{
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        MlPamFinance::where('id',$request->id)->update($production);
