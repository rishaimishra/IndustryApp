...
use App\Models\Time;
use App\Models\OtherCsiConstructionFdis;
class OtherYearlyCsiConstructionFdi extends Controller
{
    public function index()
    {
        ...
        $data['data'] = OtherCsiConstructionFdis::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->where('year',$data['get_time_data']->year)->orderBy('id','desc')->get();
        return view('csi_construction_fdi_yearly.other.index',$data);
    }
    ...
    public function insert(Request $request)
    {
        ...
        {
        $production = new OtherCsiConstructionFdis;
    ...
    {
        $data = [];
        $data['data'] = OtherCsiConstructionFdis::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('csi_construction_fdi_yearly.other.edit',$data);
        }else{
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        OtherCsiConstructionFdis::where('id',$request->id)->update($production);
