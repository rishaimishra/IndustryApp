...
use App\Models\Time;
class CostYearlyMlServiceFdi extends Controller
{
    public function index()
    {
        ...
        if (count($year_data)>0) {
            // return $year_data;
           $data['data'] = CostServiceFdis::where('year',$year)->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->where('status','!=','IP')->where('status','!=','D')->where('type','Y')->where('parent_delete_id','0')->get();
        ...
        // return $data['data'];
        return view('ml_service_fdi_yearly.cost.index',$data);
    }
    ...
    public function save_next(Request $request)
    {
        ...
        if (@$request->type_submission=="S") {
            ...
            return redirect()->route('manage.employe.manage-employment-record-ml-services-fdi-yearly');
        }
    }
    ...
    {
        $data = [];
        $data['data'] = CostServiceFdis::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('ml_service_fdi_yearly.cost.edit',$data);
        }else{
