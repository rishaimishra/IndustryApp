...
use App\Models\Time;
use App\Models\CsiPamFdiOther;
class OtherCsiPamFdiYearly extends Controller
{
    public function index()
    {
        ...
        $data['data'] = CsiPamFdiOther::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->where('year',$data['get_time_data']->year)->orderBy('id','desc')->get();
        return view('csi_pam_fdi_yearly.other.index',$data);
    }
    ...
    {
        $data = [];
        $data['data'] = CsiPamFdiOther::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('csi_pam_fdi_yearly.other.edit',$data);
        }else{
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        CsiPamFdiOther::where('id',$request->id)->update($production);
