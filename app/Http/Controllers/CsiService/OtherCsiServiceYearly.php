...
use App\Models\Time;
use App\Models\CsiServiceOther;
class OtherCsiServiceYearly extends Controller
{
    public function index()
    {
        ...
        $data['data'] = CsiServiceOther::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->where('year',$data['get_time_data']->year)->orderBy('id','desc')->get();
        return view('csi_service_yearly.other.index',$data);
    }
    ...
    public function insert(Request $request)
    {
        ...
        {
        $production = new CsiServiceOther;
    ...
    {
        $data = [];
        $data['data'] = CsiServiceOther::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('csi_service_yearly.other.edit',$data);
        }else{
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        CsiServiceOther::where('id',$request->id)->update($production);
