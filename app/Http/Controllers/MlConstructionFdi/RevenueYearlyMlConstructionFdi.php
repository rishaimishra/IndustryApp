...
use App\Models\Time;
use App\Models\RevenueYearlyMlConstructionFdis;
class RevenueYearlyMlConstructionFdi extends Controller
{
    public function index()
    {
        ...
        $data['data'] = RevenueYearlyMlConstructionFdis::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->orderBy('id','desc')->where('year',$data['get_time_data']->year)->get();
        // return $data['data'];
        return view('ml_construction_fdi_yearly.revenue.index',$data);
    }
    ...
    public function add()
    {
        ...
        $data['get_time_data'] = Time::whereDate('from_date', '<=', date('Y-m-d'))
            ->whereDate('end_date', '>=',date('Y-m-d'))->where('type','Y')->first();
        return view('ml_construction_fdi_yearly.revenue.add',$data);
    }
    ...
    public function insert(Request $request)
    {
        ...
        {
        $production = new RevenueYearlyMlConstructionFdis;
        ...
        if (@$request->type_submission=="S") {
            ...
            return redirect()->route('manage.other-ml-construction-fdi-yearly');
        }
    }
    ...
    {
        $data = [];
        $data['data'] = RevenueYearlyMlConstructionFdis::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        if($data['data']!="")
        {
            return view('ml_construction_fdi_yearly.revenue.delete',$data);
        }else{
    ...
    {
        RevenueYearlyMlConstructionFdis::where('id',$request->id)->update([
            'status'=>'D',
    ...
    public function edit($id)
    {
        ...
        if($data['data']!="")
        {
            return view('ml_construction_fdi_yearly.revenue.edit',$data);
        }else{
            return redirect()->back()->with('error','Something Went Wrong');
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        RevenueYearlyMlConstructionFdis::where('id',$request->id)->update($production);
