...
use App\Models\Time;
class CostMlConstructionFdi extends Controller
{
    public function index()
    {
        ...
        if (count($year_data)>0) {
            // return $year_data;
           $data['data'] = CostMlConstructionFdis::where('year',$year)->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->where('status','!=','IP')->where('status','!=','D')->where('type','H')->where('parent_delete_id','0')->get();
        }else{
        ...
        return view('ml_construction_fdi.cost.index',$data);
    }
    ...
    public function save_next(Request $request)
    {
        ...
        if (@$request->type_submission=="S") {
            ...
            return redirect()->route('manage.employe.manage-employment-record-ml-construction-fdi');
        }
    }
    ...
    {
        $data = [];
        $data['data'] = CostMlConstructionFdis::where('id',$id)->where('type','H')->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('ml_construction_fdi.cost.edit',$data);
        }else{
