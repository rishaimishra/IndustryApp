...
use App\Models\Time;
use App\Models\OtherYearlyMlConstructionFdis;
class OtherYearlyMlConstructionFdi extends Controller
{
    public function index()
    {
        ...
        $data['data'] = OtherYearlyMlConstructionFdis::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->where('year',$data['get_time_data']->year)->orderBy('id','desc')->get();
        return view('ml_construction_fdi_yearly.other.index',$data);
    }
    ...
    public function insert(Request $request)
    {
        ...
        {
        $production = new OtherYearlyMlConstructionFdis;
    ...
    {
        $data = [];
        $data['data'] = OtherYearlyMlConstructionFdis::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('ml_construction_fdi_yearly.other.edit',$data);
        }else{
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        OtherYearlyMlConstructionFdis::where('id',$request->id)->update($production);
