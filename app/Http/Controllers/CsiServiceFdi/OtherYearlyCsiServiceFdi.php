...
use App\Models\Time;
use App\Models\OtherYearlyCsiServiceFdis;
class OtherYearlyCsiServiceFdi extends Controller
{
    public function index()
    {
        ...
        $data['data'] = OtherYearlyCsiServiceFdis::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->where('year',$data['get_time_data']->year)->orderBy('id','desc')->get();
        return view('csi_service_fdi_yearly.other.index',$data);
    }
    ...
    public function insert(Request $request)
    {
        ...
        {
        $production = new OtherYearlyCsiServiceFdis;
    ...
    {
        $data = [];
        $data['data'] = OtherYearlyCsiServiceFdis::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('csi_service_fdi_yearly.other.edit',$data);
        }else{
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        OtherYearlyCsiServiceFdis::where('id',$request->id)->update($production);
