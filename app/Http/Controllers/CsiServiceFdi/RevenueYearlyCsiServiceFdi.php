...
use App\Models\Time;
use App\Models\RevenueYearlyCsiServiceFdis;
class RevenueYearlyCsiServiceFdi extends Controller
{
    public function index()
    {
        ...
        $data['data'] = RevenueYearlyCsiServiceFdis::where('status','!=','D')->where('profile_id',auth()->user()->current_profile)->where('user_id',auth()->user()->id)->orderBy('id','desc')->where('year',$data['get_time_data']->year)->get();
        // return $data['data'];
        return view('csi_service_fdi_yearly.revenue.index',$data);
    }
    ...
    public function insert(Request $request)
    {
        ...
        {
        $production = new RevenueYearlyCsiServiceFdis;
        ...
        if (@$request->type_submission=="S") {
            ...
            return redirect()->route('manage.other-csi-service-fdi-yearly');
        }
    }
    ...
    {
        $data = [];
        $data['data'] = RevenueYearlyCsiServiceFdis::where('id',$id)->where('user_id',auth()->user()->id)->where('status','!=','D')->first();
        
        if($data['data']!="")
        {
            return view('csi_service_fdi_yearly.revenue.edit',$data);
        }else{
    ...
    public function update(Request $request)
    {
        ...
        $production['user_id'] = auth()->user()->id;
        RevenueYearlyCsiServiceFdis::where('id',$request->id)->update($production);
